...
If the environment explicitly permits isolated subagents, run at most two row workers concurrently, exactly one job per worker. Imagegen is the worker's final user-visible output; after the worker finishes, the parent flow immediately records the worker's selected PNG. Otherwise continue sequentially without user wakeups.
